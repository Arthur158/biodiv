...
class Planet:
    ...
    def get_from_database(self):
        ...
        for autotroph_specie_data in autotroph_species_data:
            species = self.species.get(autotroph_specie_data[0])
            if isinstance(species, AutotrophSpecies):
                species.update_stats(autotroph_specie_data[1], autotroph_specie_data[2], autotroph_specie_data[3], autotroph_specie_data[4])
            else:
                continue
        ...
        for heterotroph_specie_data in heterotroph_species_data:
            species = self.species.get(heterotroph_specie_data[0])
            if isinstance(species, HeterotrophSpecies):
                species.update_stats(heterotroph_specie_data[1], heterotroph_specie_data[2], heterotroph_specie_data[3], heterotroph_specie_data[4], heterotroph_specie_data[5])
            else:
                continue
    ...
    def save_simulation(self):
        ...
        for species in self.species.values():
            if isinstance(species, AutotrophSpecies):
                self.db_handler.insert_autotroph_species(species.name, str(species.toxicity), str(species.height), str(species.depth_of_roots), str(species.size_of_leaves))
            if isinstance(species, HeterotrophSpecies):
                self.db_handler.insert_heterotroph_species(species.name, str(species.heterotroph_level), str(species.armor), str(species.speed), str(species.strength), str(species.digestive_strength), str(species.size))
        ...
        for region in self.regions.values():
            ...
            for population in region.populations.values():
                self.db_handler.insert_population(population.species.name, str(population.population_size), region.name)
    ...
    def _distribute_food_to_autotrophs(self, region, autotrophic_populations: List[Population]):
        ...
        for population in autotrophic_populations:
            ...
            if isinstance(species, AutotrophSpecies):
                gathering_powers.append(population.population_size * species.light_absorption)
            else:
                continue
        ...
        for index, population in enumerate(autotrophic_populations):
            population.food_collected += pool * (gathering_powers[index] / total_gathering_power if total_gathering_power != 0 else 0)

        #then, share resources from water
        pool = resources[1]
        ...
        for population in autotrophic_populations:
            species = population.species
            if isinstance(species, AutotrophSpecies):
                gathering_powers.append(population.population_size * species.water_absorption)
            else:
                continue
            ...
            for population in region.populations.values():
                ...
                if isinstance(population.species, HeterotrophSpecies):
                    ...
                    if(population.species.heterotroph_level > 0):
                        ...
                        ratio_grass =  (population.grass_collected * 100) / (population.population_size * population.species.calories_cost * 5 * population.species.heterotroph_level)
                    ...
                    if(population.species.heterotroph_level < 100):
                        ...
                        ratio_meat = (population.meat_collected * 100) / (population.population_size * population.species.calories_cost * 5 * (100 - population.species.heterotroph_level))
                        if ratio_meat != 0:   
                    ...
                    ratio = min(population.food_collected / (population.population_size * population.species.calories_cost), 1.0)
    ...
    def add_species(self, species: Species):
        ...
        if isinstance(species, AutotrophSpecies):
            self.db_handler.insert_autotroph_species(species.name, str(species.toxicity), str(species.height), str(species.depth_of_roots), str(species.size_of_leaves))
        elif isinstance(species, HeterotrophSpecies):
            self.db_handler.insert_heterotroph_species(species.name, str(species.heterotroph_level), str(species.armor), str(species.speed), str(species.strength), str(species.digestive_strength), str(species.size))
